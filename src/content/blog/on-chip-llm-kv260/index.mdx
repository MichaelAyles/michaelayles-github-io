...
I put a language model inside the <T id="fabric">fabric</T> of a $250 <T id="fpga">FPGA</T>. Every weight lives in on-chip memory. Nothing touches external RAM in the token loop, not the weights, not the activations, not the <T id="kv">KV cache</T>.
...
A 3.16M-parameter <T id="int4">INT4</T> transformer fits in ~1.5 MB. That's the budget met, but every byte still counts, and the last lever toward speed kept turning out to be "make the model dumber."
...
And the ceiling is proven, not assumed. There's a script that falsifies packing a third <T id="mac">MAC</T> into a <T id="dsp">DSP48E2</T> over 1.2M randomised products, and N=32 needs 2,048 DSPs the chip doesn't have. The real limit of this architecture on this silicon is 62k to 78k. 100k was a fantasy and I've disowned it.
...
It's all hand-written Verilog, no <T id="hls">HLS</T>. Claude Code did a moderate amount, it's a side project on a side project after all, though pushing an FPGA to its limit is definitely not as comfortable for it as writing a CRUD app in TypeScript.
